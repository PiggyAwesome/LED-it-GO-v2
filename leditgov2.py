...
def convert(string):
    """Convert a string into binary characters"""
    ...
    output = []
    for char in string:
        output.append(binars[char])

    binary = "".join(output)
    return binary

def leak(binary):
    """Display binary characters by flashing colours on the screen. black=0 white=1"""
    wn = turtle.Screen()
...
except FileNotFoundError as string:
    leak(convert(str(string)))
    exit()
